...
public class GebruikerInitialDataLoader {
    ...
    @Bean
    public CommandLineRunner loadData(GebruikerRepository gebruikerRepository, PasswordEncoder passwordEncoder) {
        return args -> {
            String email = "helper@helper.helper";
            if (gebruikerRepository.findByEmail(email).isEmpty()) {
                Gebruiker gebruiker1 = GebruikerBuilder.gebruikerBuilder()
            ...
            if (gebruikerRepository.findByEmail(email).isEmpty()) {
                Gebruiker gebruiker2 = GebruikerBuilder.gebruikerBuilder()
                        .setId(UUID.randomUUID())
                        .setVoornaam("looker")
